...
type Database interface {
	...
	TweetsFilteredByUser([]int) ([]ClientTweet, error)
}
...
func (d database) TweetsFilteredByUser(ids []int) ([]ClientTweet, error) {
	stmt, err := d.database.Prepare(`SELECT * FROM Tweets WHERE UserID != ANY($1);`)
	if err != nil {
		return nil, err
	}
	...
	for result.Next() {
		...
		err := result.Scan(&tempTweet.Id, &tempTweet.Time, &tempTweet.Likes, &tempTweet.UserID, &tempTweet.User, &tempTweet.Message)
		if err != nil {
			return nil, err
		}
