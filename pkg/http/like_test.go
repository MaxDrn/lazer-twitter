...
func Test_LikeHandler(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: rawMessage{
				Typ: "like",
				Msg: []byte(`{"typ":"like","username":"Peter","id":"0"`),
			},
			expectedOutput:    []byte(`{"typ":"liked","tweet":{"id":0,"time":"","likes":0,"user":"","message":""}}`),
			expectedError:     false,
			expectedLikeCalls: 1,
			returnError:       false,
		},
		{
			...
			expectedOutput:    []byte(`{"typ":"failedLike","username":"","tweetid":0}`),
			expectedError:     false,
			expectedLikeCalls: 1,
			returnError:       true,
		},
